...
serve(async (req) => {
  ...
  try {
    const { school, division, profile, goals, gpa, gender, prediction } = await req.json();
    ...
    const systemPrompt = `You are a college rowing recruitment expert who helps athletes craft professional outreach emails to coaches. All responses must be in English only.

You have extensive knowledge of NCAA rowing programs, coaching staff structures, and recruitment communication best practices.

IMPORTANT RULES:
- Coach emails MUST be realistic and verifiable. Use the standard format for college coaching staff emails.
- Most college coaches use formats like: firstname.lastname@university.edu, firstinitiallastname@university.edu, or rowing@university.edu
- Include the Head Coach and at least one assistant/recruiting coordinator when known
- Mark emails as "verified" ONLY if they follow well-known public university email patterns
- Mark as "likely" if following common patterns but not 100% certain
- NEVER fabricate personal emails - only use institutional .edu addresses
- For the email campaign, write compelling but professional emails that a high school student would realistically send
- Each email in the sequence should build on the previous one
- Include specific stats and achievements naturally
- Emails should be concise - coaches are busy people`;

    const userPrompt = `Generate coach contact information and a full recruitment email campaign for:
...
${prediction ? `- Chance at this school: ${prediction.chance || "Unknown"}` : ""}

Generate realistic coach emails and a 4-email recruitment campaign sequence for this specific school.`;
